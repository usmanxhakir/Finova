...
export function RecordPaymentModal({ invoice, bankAccounts, onRecord, onSuccess, trigger }: RecordPaymentModalProps) {
    ...
    const form = useForm<PaymentFormValues>({
        ...
        defaultValues: {
            ...
            amount: Number((invoice as any).amount_due || 0) / 100,
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild>
                {trigger || <Button className="bg-green-600 hover:bg-green-700 text-white">Record Payment</Button>}
            </DialogTrigger>
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    <DialogTitle>Record Payment</DialogTitle>
                    <DialogDescription>
                        Record a payment for Invoice {(invoice as any).number}.
                        Current balance due: {formatCurrency(Number((invoice as any).amount_due || 0))}
                    </DialogDescription>
                </DialogHeader>
                <Form {...form}>
